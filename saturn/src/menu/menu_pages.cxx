/*----------------------
 ...
 |   Controls views (gamepad and keyboard), Sound Options, Display Options,
 |   and the read-only, paginated Credits page. Every page constructs a
...
/*----------------------
 | credits_page
 | Description: Read-only, paginated Credits page reached from the bottom of
 |   the Options menu. Left/Right step between CREDITS_PAGES entries (clamped,
 |   not wrapping, so the page-count indicator stays a reliable "how much is
 |   left" cue); any of B/A/C/Start/Enter/Esc/Backspace closes it, since there
 |   is nothing here to confirm or cancel -- unlike the editable pages, every
 |   exit is the same exit. Draws the full 40-column width so the credit text
 |   need not compete with a value column.
 | Author: suinevere
...
static void credits_page(void) {
    ...
    for (;;) {
        ...
        for (int i = 0; i < cp.n; i++) SRL::Debug::Print(x, y++, "%s", cp.lines[i]);
