...
Every artifact is named from `CD_NAME` in `saturn/makefile` (currently
`Zaturn (USA) (Netlink Edition)`). This produces
`saturn/BuildDrop/Zaturn (USA) (Netlink Edition).iso` (bootable, ISO9660) and
`.bin` (MODE1/2352 raw, for ODEs/burners), plus the matching `.cue`, `.elf`, and
`.map`. `./clean.bat` removes build output.
...
> `saturn/src/video/display.h` (and check `g_image_name`'s fixed-size arrays in
> `saturn/src/video/title.cxx` alongside it), or retire a background.
...
| <a href="https://unsplash.com/photos/abandoned-underground-bunker-with-old-industrial-equipment-and-a-table-txBz9oWJUsM">Peter Herrmann</a> | Bunker image, Castle image |
| <a href="https://unsplash.com/photos/black-and-gray-typewriter-on-table-DWyRC2juMgs">Mike Hindle</a> | Typewriter image |
| <a href="https://unsplash.com/photos/grayscale-photography-of-tomb-eWhUCWeSTA0">Agto Nugroho</a> | Temple image |
| <a href="https://unsplash.com/photos/forest-scenery-uwLXzP9U9w4">Maxx Gong</a> | Forest image |
| <a href="https://unsplash.com/photos/person-standing-on-rock-formation-during-daytime-3-lOOy4_mxo">Taneli Lahtinen</a> | Cliff image |
