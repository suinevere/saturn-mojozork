...
while IFS= read -r line; do
  case "$line" in
    *TRACK\ *) tnum=$(echo "$line" | sed -E 's/.*TRACK 0*([0-9]+).*/\1/');
               ...
               # --- DEBUG TEST ---
               echo "$line" | grep -qE 'AUDIO|MODE1|MODE2' || echo "[DEBUG ERROR] ttype grep matched nothing on: $line" >&2

               # '|| true' added to prevent set -e silent crash
               ttype=$(echo "$line" | grep -oE 'AUDIO|MODE1|MODE2' || true);;

    *INDEX\ 01\ *) local msf;

                   # --- DEBUG TEST ---
                   echo "$line" | grep -qE '[0-9]{2}:[0-9]{2}:[0-9]{2}' || echo "[DEBUG ERROR] msf grep matched nothing on: $line" >&2

                   # '|| true' added to prevent set -e silent crash
                   msf=$(echo "$line" | grep -oE '[0-9]{2}:[0-9]{2}:[0-9]{2}' || true);
...
for ((i=0;i<n;i++)); do
  ...
  if [ -z "$count" ] || [ "$count" -lt 0 ]; then
    echo "[DEBUG ERROR] Invalid dd count ($count) for $name (startf: $startf, endf: $endf)" >&2
  fi

  # Captured 2> to a log file instead of /dev/null, and added an error echo on failure
  dd if="$bin" of="$out/$name" bs=2352 skip="$startf" count="$count" 2>>"$out/dd_debug.log" || echo "[DEBUG ERROR] dd failed on $name, check $out/dd_debug.log" >&2
